...
export function useLatestOutgoingMessageId(opts: {
  ...
  return React.useMemo(() => {
    ...
    for (const m of messages) {
      ...
      const isOutgoingByUserSub = !!myUserId && !!m.userSub && String(m.userSub) === String(myUserId);
      const isEncryptedOutgoing = !!m.encrypted && !!myPublicKey && m.encrypted.senderPublicKey === myPublicKey;
      const isPlainOutgoing =
        !m.encrypted && (isOutgoingByUserSub ? true : normalizeUser(m.userLower ?? m.user ?? 'anon') === myLower);
